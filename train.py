    ...
    for epoch in range(epoch_start, epochs + 1):
        ...
        if epoch % image_log_interval == 0:
            ...
            if model.model_name in ae_vine_models:
                ...
                copula_controls = pv.FitControlsVinecop(family_set=[pv.BicopFamily.tll], trunc_lvl=5, num_threads = cores)
                #vine_obj = rvinecop.vine(features, copula_controls=copula_controls)
                vine_obj = pv.Vinecop(np.array(features), controls = copula_controls)
